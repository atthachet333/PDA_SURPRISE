...
export function FloatingContact() {
  ...
  const channels: Channel[] = [
    ...
    { icon: 'calendar', eyebrow: t(dock.consultEyebrow), label: t(dock.consultLabel), to: cta.consult.to }
  ];
  ...
  return (
    <>
      ...
      <div className="floating-contact-position fixed z-[55]">
            ...
            >
              ...
              <div className="relative overflow-hidden rounded-card border border-brand-400/15 bg-white/[.04] p-4">
                ...
                <div className="relative flex items-start justify-between gap-4">
                  ...
                  <button type="button" onClick={() => setOpen(false)} aria-label={t(dock.closeShort)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/10 text-white/65 transition hover:bg-white/10 hover:text-white">×</button>
                </div>
              </div>
        ...
        <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label={t(open ? dock.close : dock.open)} className="group ml-auto flex h-13 w-13 items-center justify-center gap-2.5 rounded-full border border-brand-400/35 on-dark bg-[linear-gradient(150deg,#063B2A,#031b13)] text-white shadow-lift transition-all duration-base hover:-translate-y-1 hover:border-brand-300/60 hover:shadow-brand-glow sm:h-14 sm:w-auto sm:rounded-pill sm:px-5">
