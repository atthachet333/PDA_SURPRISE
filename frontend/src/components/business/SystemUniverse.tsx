...
import { systemCategoryName } from '@/i18n/systemUniverse';
import { solutionsPage as copy } from '@/i18n/solutionsPage';
...
export function SystemUniverse() {
  ...
  const labelId = useId();
  const { t } = useLocale();
  /* Same ids, targets and positions as the Thai record; words in the active locale. */
  const businessSystems = useSystems();
  const getSystemById = (id: string) => businessSystems.find((system) => system.id === id);
  const edgeLabel = (from: string, to: string, fallback: string) =>
    getSystemById(from)?.connections.find((connection) => connection.targetId === to)?.label ?? fallback;
  const [mapLead, mapAccent] = t(copy.mapTitle);
    ...
    <section aria-labelledby={labelId} className="sect sect--mesh relative overflow-hidden py-section text-white">
      ...
      <Container wide className="relative">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <p className="max-w-lg text-sm leading-relaxed text-brand-100/70">{t(copy.mapLead)}</p>
        </div>

        <div className="mt-10 grid gap-6 xl:grid-cols-[minmax(0,1.42fr)_minmax(20rem,.58fr)]">
          <div className="hidden min-h-[42rem] rounded-panel border border-brand-400/20 bg-brand-900/65 p-6 lg:block" aria-label={t(copy.mapAria)}>
            <div className="relative h-full min-h-[38rem]">
              ...
              <div className="absolute left-1/2 top-1/2 flex h-28 w-28 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-full border border-brand-300/60 bg-brand-700 text-center shadow-brand-glow" aria-label={t(copy.coreAria)}><span className="font-mono text-[.55rem] tracking-[.24em] text-brand-200">ECOSYSTEM</span><strong className="mt-1 text-lg">PDA CORE</strong><span className="mt-1 text-[.6rem] text-brand-100/70">SHARED FLOW</span></div>
          ...
          <div className="lg:hidden">
            <p className="mb-3 text-xs text-brand-100/60">{t(copy.tapHint)}</p>
          ...
          <SystemDetail system={selected} edges={relatedEdges.map((edge) => ({ label: edgeLabel(edge.from, edge.to, edge.label), system: getSystemById(edge.from === selected.id ? edge.to : edge.from) })).filter((item): item is { label: string; system: BusinessSystem } => Boolean(item.system))} />
        </div>
      </Container>
    </section>
  );
...
function SystemDetail({ system, edges }: { system: BusinessSystem; edges: { label: string; system: BusinessSystem }[] }) {
  ...
  return (
    <aside key={system.id} aria-live="polite" className="rounded-panel border border-brand-400/25 bg-brand-900 p-5 sm:p-7">
      ...
      <div className="mt-6 border-t border-brand-400/15 pt-5"><p className="font-mono text-[.58rem] tracking-[.16em] text-brand-300">CAN CONNECT</p>{edges.length ? <ul className="mt-3 space-y-3">{edges.map(({ system: peer, label }) => <li key={peer.id} className="rounded-card border border-brand-400/15 bg-brand-800/55 p-3"><span className="text-xs font-bold text-white">{peer.nameEn}</span><span className="mt-1 block text-[.7rem] leading-relaxed text-brand-100/60">{label}</span></li>)}</ul> : <p className="mt-3 text-xs leading-relaxed text-brand-100/60">{t(copy.standalone)}</p>}</div>
      <div className="mt-6 flex flex-wrap gap-2">
        {caseStudy ? <Link to={`/work/${caseStudy.slug}`} className="inline-flex min-h-11 items-center gap-2 rounded-pill bg-white px-5 py-2.5 text-sm font-semibold text-brand-800 transition-colors hover:bg-brand-50">{t(copy.viewExample)} <ArrowIcon /></Link> : null}
        <Link to={system.route} className="inline-flex min-h-11 items-center gap-2 rounded-pill border border-brand-300/30 px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-white/5">{t(copy.viewService)}</Link>
        <Link to={contactTo} className="inline-flex min-h-11 items-center gap-2 rounded-pill border border-brand-300/30 px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-white/5">{t(copy.consult)}</Link>
      </div>
    </aside>
  );
}
