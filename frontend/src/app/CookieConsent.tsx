...
function CookieConsentUI({ consent, settingsOpen, onOpenSettings, onCloseSettings, onSave }: { consent: CookiePreferences | null; settingsOpen: boolean; onOpenSettings: () => void; onCloseSettings: () => void; onSave: (value: CookiePreferences) => void }) {
  ...
  return (
    <>
      <AnimatePresence>{!consent && !settingsOpen ? <motion.aside role="region" aria-label={t(cookie.region)} initial={reduced ? { opacity: 0 } : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: reduced ? 0 : .36 }} className="fixed inset-x-3 bottom-[calc(1rem+env(safe-area-inset-bottom))] z-[70] overflow-hidden rounded-panel border border-brand-200 bg-white/95 p-5 shadow-lift-lg backdrop-blur-xl sm:left-6 sm:right-auto sm:w-[32rem] sm:p-6"><span aria-hidden="true" className="absolute inset-x-0 top-0 h-0.5 origin-left animate-[reveal-line-up_.6s_ease-out_both] bg-brand-500" /><div className="flex items-start gap-3"><Logo compact /><div><h2 className="thai-display text-base font-bold text-ink">{t(cookie.title)}</h2><p className="mt-2 text-xs leading-6 text-steel-600">{t(cookie.body)}</p></div></div><div className="mt-5 flex flex-col gap-2 sm:flex-row sm:flex-wrap"><button type="button" onClick={() => onSave({ essential: true, analytics: true, preferences: true })} className="h-11 rounded-pill bg-brand-700 px-5 text-sm font-semibold text-white hover:bg-brand-800">{t(cookie.acceptAll)}</button><button type="button" onClick={() => onSave({ essential: true, analytics: false, preferences: false })} className="h-11 rounded-pill border border-steel-300 px-5 text-sm font-semibold text-ink hover:bg-steel-50">{t(cookie.necessaryOnly)}</button><button type="button" onClick={onOpenSettings} className="h-11 rounded-pill px-4 text-sm font-semibold text-brand-700 hover:bg-brand-50">{t(cookie.settings)}</button></div><LocaleLink to="/cookie-policy" className="mt-4 inline-block text-xs text-steel-500 underline decoration-steel-300 underline-offset-4 hover:text-brand-700">{t(cookie.policy)}</LocaleLink></motion.aside> : null}</AnimatePresence>
...
function CookieSettings({ initial, onClose, onSave, reduced }: { initial: CookiePreferences | null; onClose: () => void; onSave: (value: CookiePreferences) => void; reduced: boolean }) {
  ...
  return <motion.div className="fixed inset-0 z-[80] flex items-end justify-center bg-ink/45 p-0 backdrop-blur-sm sm:items-center sm:p-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><motion.div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="cookie-settings-title" initial={reduced ? { opacity: 0 } : { opacity: 0, y: 22, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12 }} transition={{ duration: reduced ? 0 : .32 }} className="max-h-[82dvh] w-full max-w-xl overflow-y-auto rounded-t-panel border border-brand-200 bg-white p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] shadow-lift-lg sm:rounded-panel sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="section-code">COOKIE SETTINGS</p><h2 id="cookie-settings-title" className="thai-display mt-3 text-xl font-bold text-ink">{t(cookie.settingsTitle)}</h2></div><button type="button" onClick={onClose} aria-label={t(cookie.closeSettings)} className="flex h-10 w-10 items-center justify-center rounded-full border border-steel-200 text-steel-600 hover:bg-steel-50">×</button></div><div className="mt-6 divide-y divide-steel-200 border-y border-steel-200"><PreferenceRow title={t(cookie.essentialTitle)} description={t(cookie.essentialBody)} checked disabled onChange={() => undefined} /><PreferenceRow title={t(cookie.analyticsTitle)} description={t(cookie.analyticsBody)} checked={analytics} onChange={setAnalytics} /><PreferenceRow title={t(cookie.preferencesTitle)} description={t(cookie.preferencesBody)} checked={preferences} onChange={setPreferences} /></div><div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={onClose} className="h-11 rounded-pill px-5 text-sm font-semibold text-steel-600 hover:bg-steel-50">{t(cookie.cancel)}</button><button type="button" onClick={() => onSave({ essential: true, analytics, preferences })} className="h-11 rounded-pill bg-brand-700 px-6 text-sm font-semibold text-white hover:bg-brand-800">{t(cookie.save)}</button></div></motion.div></motion.div>;
}
